...
import { ArrowLeft, Undo2, Redo2, List, Search, ChevronDown } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="bg-[color:var(--card-background)] rounded-lg p-4 shadow-lg mb-4 border border-[color:var(--card-border)]">
        <textarea
          ...
          className={`w-full text-xl font-bold text-[color:var(--text-primary)] placeholder-[color:var(--text-muted)] outline-none mb-2 bg-transparent border-none resize-none overflow-hidden leading-tight ${
            tituloFocused ? 'ring-2 ring-[color:var(--primary)] rounded px-2 py-1' : 'focus:ring-0'
          }`}
          style={{
            minHeight: '1.5rem',
            height: 'auto'
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex gap-1 bg-[color:var(--card-background)] rounded-lg p-1 border border-[color:var(--card-border)] shadow-lg">
          <button
            onClick={deshacer}
            disabled={indiceHistorial <= 0}
            className="p-2 text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] hover:bg-[color:var(--hover-bg)] rounded transition-all disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        {modulos.length > 0 && (
          <div className="flex-1 relative">
            <button
              ...
              className="w-full px-3 py-2 bg-[color:var(--card-background)] border border-[color:var(--card-border)] rounded-lg text-left text-sm text-[color:var(--text-primary)] hover:border-[color:var(--primary)] transition-all flex items-center justify-between shadow-lg"
            >
              <span className="flex items-center gap-2">
                <List size={16} />
                <span>Índice de preguntas ({obtenerTodasLasPreguntas().length})</span>
              </span>
              <ChevronDown size={16} className={`transition-transform ${indiceAbierto ? 'rotate-180' : ''}`} />
            </button>
                    ...
                    .map((pregunta) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-[color:var(--text-muted)] font-semibold w-6">P{pregunta.numero}</span>
                          <span className="text-[color:var(--text-secondary)] font-mono text-xs bg-[color:var(--hover-bg)] px-1.5 py-0.5 rounded">
                            {pregunta.value}
                          </span>
                          <span className="text-[color:var(--text-primary)] truncate flex-1">{pregunta.text}</span>
                          {pregunta.tieneCondiciones && (
                            <span className="text-purple-400 text-xs">🔗</span>
                          )}
                        </div>
                        <div className="text-[color:var(--text-muted)] text-xs mt-0.5 ml-8">{pregunta.moduloNombre}</div>
                      </button>
                    ))
                  }
                </div>
